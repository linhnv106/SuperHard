package com.mk.apps.superm.tiledMapGame.screens;
...
import com.mk.apps.superm.tiledMapGame.entities.Player;
import com.mk.apps.superm.tiledMapGame.entities.Player.IPlayerListener;
...
//
		...
		font.draw( levelBatch,lv, Gdx.graphics.getWidth()/2-100,Gdx.graphics.getHeight()- 50);
		levelBatch.end();
...
//			}
	...
	public void show() {
		...
		player.setPosition(12 * player.getCollisionLayer().getTileWidth(), (player.getCollisionLayer().getHeight() - 4) * player.getCollisionLayer().getTileHeight());
		batch= new SpriteBatch();
		bgTexture = new Texture(Gdx.files.internal("testbg.png"));

		
		bgTexture.setFilter(TextureFilter.Linear, TextureFilter.Linear);
		TextureRegion region = new TextureRegion(bgTexture, 0, 0, 1280, 800);
		...
		// get the frame tiles
//		Iterator<TiledMapTile> tiles = map.getTileSets().getTileSet("tileSet-hd").iterator();
//		while(tiles.hasNext()) {
//			TiledMapTile tile = tiles.next();
//			if(tile.getProperties().containsKey("animation") && tile.getProperties().get("animation", String.class).equals("moving"))
...
//				if(cell!=null){
//				if(cell.getTile().getProperties().containsKey("animation") && cell.getTile().getProperties().get("animation", String.class).equals("moving"))
